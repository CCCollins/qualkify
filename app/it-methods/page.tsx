...
export default function Home() {
  return (
    <main className="bg-white sm:bg-gradient-to-br sm:from-purple-100 sm:to-pink-200 flex flex-col items-center justify-center px-6 py-12">
      <div className="text-center mb-4">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-2">
          <Link href="/">
            <span className="bg-purple-600 text-white px-4 py-1 rounded-xl shadow transition">
              Qualkify
            </span>
          </Link>
        </h1>
        ...
        {modules.length > 0 ? (
          modules.map(({ name, path, emoji }) => (
            <Link
              key={name}
              href={path}
              className="w-full flex items-center gap-4 px-6 py-5 bg-white text-purple-800 font-semibold rounded-xl shadow-md hover:shadow-lg hover:bg-purple-50 border border-purple-100 transition-all duration-200 text-left"
            >
